#include "../include/hp_server_socket.h"

hp_server_socket::hp_server_socket(int port, int domain, int type, int protocol, int num_front/* = 5*/)
{
	if (domain != AF_INET) {
...
hp_server_socket::hp_server_socket(int domain, int type, int protocol, std::string listen_on, int num_front/* = 5*/)
{
	if (domain != AF_UNIX) {
...
hp_server_socket::~hp_server_socket()
{

}
int hp_server_socket::get_server_socket()
{
	return m_server_sockfd;
}
void hp_server_socket::create_socket()
{
	...
	if (m_domain == AF_INET) {
		...
		if (bind(m_server_sockfd, (struct sockaddr *)&m_server_address, m_server_len) < 0) {
			std::cerr << "cannot bind socket"<<" on domain = "<<m_domain<<std::endl; 
			exit(1); 
		}
		...
		if (bind(m_server_sockfd, (struct sockaddr *)&m_server_unixaddress, m_server_len) < 0) {
			std::cerr << "cannot bind unix socket"<<" on domain = "<<m_listen_on<<std::endl; 
			exit(1); 
		}
	}
...
std::string hp_server_socket::accept_socket()
{
        char buffer[MAX_MSG];
	std::string data;
	std::string comm;
...
void hp_server_socket::close_client_socket()
{
	shutdown(m_client_sockfd,2);
	close(m_client_sockfd);
}
void hp_server_socket::close_server_socket()
{	
